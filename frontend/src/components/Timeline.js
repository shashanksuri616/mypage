                ...
                {timeline.map((item, i) => (
                    <motion.div
                        ...
                        className={`flex items-start mb-12 group transition-transform duration-300 ${
                            item.projectId ? 'cursor-pointer hover:scale-[1.07] hover:shadow-2xl' : ''
                        }`}
                    ...
                    >
                        ...
                        <div className="flex flex-col items-center mr-6 relative">
                            <motion.div
                                className={`w-9 h-9 rounded-full shadow-lg border-2 border-white dark:border-gray-900 mb-1 flex items-center justify-center bg-gradient-to-br ${COLORS[i % COLORS.length]}`}
                                animate={open === i ? { scale: 1.28, boxShadow: "0 0 0 10px #fbbf2466" } : { scale: 1, boxShadow: "0 2px 12px #a78bfa55" }}
