...
// Add a rainbow gradient array for more color variety
const GRADIENTS = [
            ...
            <div className="flex flex-wrap justify-center gap-8">
                {skills.map((skill, i) => (
                    <motion.div
                        ...
                        whileHover={{ scale: 1.11, boxShadow: "0 8px 32px #a78bfa33" }}
                        onClick={() => setOpen(i)}
                        tabIndex={0}
                        onKeyDown={e => { if (e.key === "Enter" || e.key === " ") setOpen(i); }}
                        aria-expanded={open === i}
                    >
                        <div className={`absolute inset-0 rounded-2xl opacity-30 pointer-events-none bg-gradient-to-br ${GRADIENTS[i % GRADIENTS.length]}`} />
                        <span className="text-5xl mb-4 drop-shadow-lg relative z-10">{skill.icon}</span>
                        <span className="font-semibold text-lg text-purple-700 dark:text-yellow-200 relative z-10">{skill.name}</span>
                    </motion.div>
                ))}
            </div>
