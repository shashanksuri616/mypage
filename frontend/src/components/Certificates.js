...
// Rainbow gradient backgrounds for more visual pop
const GRADIENTS = [
...
const Certificates = () => {
    ...
    return (
        <section
            id="certificates"
            className="py-24 px-6 bg-gradient-to-br from-white/60 via-purple-50/60 to-yellow-50/40 dark:from-[#18122b]/80 dark:via-[#312e81]/70 dark:to-[#000]/60 backdrop-blur-md text-black dark:text-white transition-colors duration-300"
        >
            ...
            <div className="flex flex-wrap justify-center gap-10">
                {certificates.map((cert, i) => (
                    <motion.a
                        ...
                        key={cert.name}
                        className="relative block bg-white/70 dark:bg-gray-800/70 border-2 border-transparent hover:border-purple-400 shadow-xl rounded-xl transition-colors duration-300 p-4 w-64 cursor-pointer overflow-hidden"
                        ...
                        aria-expanded={selected && selected.name === cert.name}
                    >
                        <div className={`absolute inset-0 rounded-xl opacity-25 pointer-events-none bg-gradient-to-br ${GRADIENTS[i % GRADIENTS.length]}`} />
                        <img
                            src={cert.img}
                            alt={cert.name}
                            className="w-full h-36 object-contain mb-4 rounded relative z-10"
                        />
                        <div className="font-semibold text-lg relative z-10">{cert.name}</div>
                        <div className="text-purple-600 relative z-10">{cert.org}</div>
                        <div className="text-xs text-gray-500 mt-1 relative z-10">{cert.year}</div>
                        <div className="flex flex-wrap gap-1 mt-2 relative z-10">
                            {cert.skills.map((skill) => (
                                <span
                                    key={skill}
                                    className="bg-purple-100 dark:bg-purple-900 text-purple-700 dark:text-purple-200 px-2 py-0.5 rounded text-xs"
                                >
                                    {skill}
                                </span>
                            ))}
                    ...
                    >
                        ...
                        >
                            ...
                            <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="inline-block mt-4 px-4 py-2 bg-gradient-to-r from-purple-600 via-fuchsia-500 to-yellow-400 hover:brightness-110 text-white rounded-lg transition"
                            >
                                View Certificate
                            </a>
                        </motion.div>
