...
const Navbar = () => {
	...
	// Close mobile menu on nav click or resize
	useEffect(() => {
			...
			<AnimatePresence>
				{show && (
                  ...
                  `}
						...
						{/* Mobile Nav Toggle */}
						<button
							className="sm:hidden flex items-center justify-center w-10 h-10 rounded-full bg-white/80 dark:bg-gray-900/80 border border-gray-200 dark:border-gray-700 shadow-lg ml-2"
							onClick={() => setMobileOpen((o) => !o)}
							aria-label="Open navigation"
						>
							<svg
								className="w-7 h-7 text-purple-700 dark:text-purple-200"
								fill="none"
								stroke="currentColor"
								strokeWidth="2"
								viewBox="0 0 24 24"
			...
			<AnimatePresence>
				{mobileOpen && (
					<motion.div
						...
						className="fixed top-16 left-0 right-0 z-50 flex flex-col items-center bg-white/95 dark:bg-gray-900/95 backdrop-blur-lg shadow-lg py-4 gap-2 border-b border-purple-200 dark:border-purple-900 sm:hidden"
					>
						{navItems.map((item) => (
							<button
								key={item.id}
